...
class WC_SF_Settings extends WC_Settings_Page {
	...
	public function get_settings( $current_section = '' ) {
		...
		switch ( $current_section ) {
			...
			case 'api_log':
				...
				if ( $api_usage ) {
					if ( time() < strtotime( $api_usage['dailyreset'] ) ) {
						$content .= '<div class="sf-notice-' . (( 0 >= $api_usage['dailyremaining'] ) ? 'error' : 'info') . '">';
						$content .= sprintf( wp_kses( '<big>%s: %d/%d (' . __( '%d remaining', 'woocommerce-superfaktura' ) . ')</big>', $this->allowed_tags ),
							__( 'Daily API usage', 'woocommerce-superfaktura' ),
							min( $api_usage['dailylimit'], $api_usage['dailylimit'] - $api_usage['dailyremaining'] ),
							$api_usage['dailylimit'],
							max( 0, $api_usage['dailyremaining'] )
						);

						if ( 0 >= $api_usage['dailyremaining'] ) {
							$content .= '<p>' . __( 'You have exceeded the allowed number of API requests / day. If you issue a large number of invoices, you can increase this number.', 'woocommerce-superfaktura' ) . '</p>';
							$content .= '<a class="button button-primary button-hero" href="https://moja.superfaktura.sk/shoppings/index/api_limit_increase/recommended_increase:' . ( ceil ( ( abs( $api_usage['dailyremaining'] ) + 100 ) / 1000 ) * 1000 ) . '/recommended_duration:8">' . __( 'Increase the number of API requests', 'woocommerce-superfaktura' ) . '</a>';
						}

						$content .= '</div>';
					}
					else {
						$content .= '<div class="sf-notice-info">';
						$content .= sprintf( '<big>%s: %d/%d (%d remaining)</big>',
							__( 'Daily API usage', 'woocommerce-superfaktura' ),
							0,
							$api_usage['dailylimit'],
							$api_usage['dailylimit']
				...
				$settings = array(
					...
					array(
						...
						'desc'  => '
							...
							<span class="wc-sf-api-test-loading"><img src="' . plugins_url( 'assets/images/rolling.gif', WC_SF_FILE_PATH ) . '" width="25" height="25" alt=""></span>
							<span class="wc-sf-api-test-ok"><img src="' . plugins_url( 'assets/images/ok.png', WC_SF_FILE_PATH ) . '" width="32" height="32" alt=""></span>
							<span class="wc-sf-api-test-fail"><img src="' . plugins_url( 'assets/images/fail.png', WC_SF_FILE_PATH ) . '" width="32" height="32" alt=""></span>
	...
	public function output() {
		...
		if ( 'yes' === get_option( 'woocommerce_sf_sandbox', 'no' ) ) {
			echo wp_kses( '<div class="sf-notice-info">' . __( 'Plugin SuperFaktura WooCommerce is in sandbox mode.', 'woocommerce-superfaktura' ) . '</div>', $this->allowed_tags );
		}
