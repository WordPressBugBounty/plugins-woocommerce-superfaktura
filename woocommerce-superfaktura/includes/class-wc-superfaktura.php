...
class WC_SuperFaktura {
	...
	public function admin_enqueue_scripts() {
		wp_enqueue_script( 'wc-sf-admin-js', plugins_url( 'assets/js/admin.js', WC_SF_FILE_PATH ), array( 'jquery' ), $this->version, true );
	...
	public function admin_init() {
		if ( isset( $_GET['sf_regen'] ) || isset( $_GET['sf_invoice_proforma_create'] ) || isset( $_GET['sf_invoice_regular_create'] ) || isset( $_GET['sf_invoice_cancel_create'] ) ) {
			...
			if ( isset( $_GET['sf_regen'] ) ) {
				...
				$result      = $this->invoice_generator->generate_invoice( $order, 'proforma', isset( $_GET['force_create'] ) );
				$result_msg .= 'proforma';
			} elseif ( isset( $_GET['sf_invoice_regular_create'] ) ) {
				$order       = wc_get_order( $order_id );
				$result      = $this->invoice_generator->generate_invoice( $order, 'regular', isset( $_GET['force_create'] ) );
				$result_msg .= 'regular';
			} elseif ( isset( $_GET['sf_invoice_cancel_create'] ) ) {
				$order       = wc_get_order( $order_id );
				$result      = $this->invoice_generator->generate_invoice( $order, 'cancel', isset( $_GET['force_create'] ) );
				$result_msg .= 'cancel';
			}
	...
	public function admin_notices() {
		...
		if ( $api_usage && 0 >= $api_usage['dailyremaining'] && time() < strtotime( $api_usage['dailyreset'] ) ) {
			...
			echo sprintf( wp_kses( '<div class="notice notice-error"><p>%s</p></div>', $this->allowed_tags ), $api_usage_notice );
		}
	...
	public function add_box( $post ) {
		...
		if ( ( ! empty( $proforma ) || ! empty( $invoice ) ) && $this->invoice_generator->sf_can_regenerate( $order ) ) {
