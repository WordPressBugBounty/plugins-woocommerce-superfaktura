    ...
    */
    ...
    public function get($url, $json_decode = true)
    {
        try {
            ...
            $this->saveApiUsageInfo($response['headers']);
    ...
    /**
     * Save API usage information to the database
     *
     * @param array $response_headers
     *
     * @return bool
     */
    private function saveApiUsageInfo($response_headers) {
        return update_option('woocommerce_sf_api_usage', [
            'dailylimit'     => $response_headers['x-ratelimit-dailylimit'] ?? 1000,
            'dailyremaining' => $response_headers['x-ratelimit-dailyremaining'] ?? 1000,
            'dailyreset'     => $response_headers['x-ratelimit-dailyreset'] ?? date('d.m.Y H:i:s'),
            'message'        => $response_headers['x-ratelimit-message'] ?? ''
        ]);
    }
}

class WC_SF_Api_Cz extends WC_SF_Api
{
    const SFAPI_URL = 'https://moje.superfaktura.cz';
    const SANDBOX_URL = 'https://sandbox.superfaktura.cz';
}

class WC_SF_Api_At extends WC_SF_Api
{
