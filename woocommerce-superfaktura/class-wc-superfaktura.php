	...
	public function sf_generate_invoice( $order, $type, $force_create = false ) {
		...
		try {
			...
			if ( 'yes' === get_option( 'woocommerce_sf_comments' ) ) {
				...
				if ( ( $client_data['ic_dph'] && WC()->countries->get_base_country() !== $order->get_billing_country() ) || ( in_array( WC()->countries->get_base_country(), array( 'SK', 'CZ' ), true ) && $order->get_billing_country() && ! in_array( $order->get_billing_country(), $this->eu_countries, true ) ) ) {
